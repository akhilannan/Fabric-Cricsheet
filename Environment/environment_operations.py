    ...
    try:
        ...
        if environment_id:
            ...
            if response.status_code == 200:
                ...
                while True:
                    publish_state = get_publish_state(
                        environment_id, workspace_id, client=client
                    )
...
def upload_files_to_environment(
    environment_id: str,
    file_paths: list[str],
    workspace: str = None,
                ...
                try:
                    with open(file_path, "rb") as file:
                        ...
                        if response.status_code == 200:
                            ...
                            results[file_name] = (
                                f"Error uploading library: {response.text}"
                            )
                except Exception as e:
                    results[file_name] = f"Error: {str(e)}"
        else:
            results["error"] = "Environment ID not provided."
    except Exception as e:
...
def delete_existing_libraries_and_publish(
    ...
    base_endpoint = f"/v1/workspaces/{workspace_id}/environments/{environment_id}"
    ...
    try:
        ...
        files_to_delete = [
            file
            for file_type in ["wheelFiles", "pyFiles", "jarFiles", "rTarFiles"]
        ...
        if files_to_delete:
            ...
            publish_message = publish_staging_environment(
                environment_id, workspace_id, client=client
            )
...
def create_and_publish_spark_environment(
    ...
    """
    workspace_id = resolve_workspace_id(workspace, client=client)
    environment_id = get_create_or_update_fabric_item(
    ...
    print(
        upload_files_to_environment(
            environment_id, py_path, workspace_id, client=client
        )
    )
...
def create_or_replace_notebook_from_ipynb(
    notebook_path: str,
    ...
    if default_lakehouse_name:
        default_lakehouse_id = create_lakehouse_if_not_exists(
            default_lakehouse_name, workspace_id, client=client
        )
    ...
    if environment_name:
        environment_id = get_item_id(
            environment_name, "Environment", workspace_id, client=client
        )
